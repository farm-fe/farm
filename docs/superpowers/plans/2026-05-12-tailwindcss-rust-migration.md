...
| Phase 13: Candidate parser parity | ✅ **Done** | Negative utilities (`-mt-4`, `hover:-mt-4`, `!-mt-4`); underscore→space normalisation in arbitrary values + arbitrary properties (preserves `\_` escapes and underscores inside `url(...)`); paren-arbitrary CSS-variable shorthand `bg-(--my-color)` → `var(--my-color)` (with `\_` decode + optional type hint `bg-(color:--my-color)`); paren-aware `:` variant splitter; 14 new tests |
...
| Phase 16: Utilities (~560 registrations) | 🟡 **In progress** | Functional-utility infrastructure landed: `FunctionalHandler` registry, multi-segment key resolution (e.g. `rounded-tl-lg` → handler `rounded-tl` + leftover `lg`), `Theme::with_defaults()` seeded with the v4 spacing base + default color palette (black/white/transparent/current + slate/gray/red/blue/green/yellow/indigo) + radius/text/font-weight/aspect scales. Implemented categories: **spacing** (m/mt/mr/mb/ml, mx/my, ms/me, p variants, ps/pe, gap, gap-x/y, inset, inset-x/y, top/right/bottom/left, start/end), **sizing** (w/h/size, min-w/h, max-w/h with `full`/`auto`/`screen`/`dvh`/`svh`/`lvh`/`dvw`/`svw`/`lvw`/`min`/`max`/`fit`/`px`/fractions/arbitrary), **colors** with `/<opacity>` modifier (`text`/`bg`/`border`/`fill`/`stroke`/`ring`/`accent`/`caret`/`decoration`/`outline`/`placeholder`) using `color-mix(in oklab, …, transparent)`, **border radius** (`rounded` + `-t/r/b/l/tl/tr/bl/br`), **border width** (`border-t/r/b/l/x/y` + numeric sizes), **font size** (`text-<size>` with nested line-height), **opacity**, **z-index** (with negative), **order** (first/last/none/numeric), **flex grow/shrink/basis**, **list style type**, **line clamp**, **columns**, **aspect ratio**, and **grid placement/tracks** (`col-*`, `row-*`, `auto-cols-*`, `auto-rows-*`, `grid-flow-*`, `grid-cols-*`, `grid-rows-*`). 46 new tests. Remaining lower-frequency utilities (background images/gradients, shadows, transforms, animations, transitions, filter/backdrop-filter, mask, scroll-*, place-* alignments, font-stretch/font-variant-numeric, breaks, isolation, mix-blend, will-change, content, etc.) are deferred to Phase 16.x sub-PRs. |
...
| Phase 23: Utility follow-up — list/line-clamp/columns/aspect | ✅ **Done** | Added lower-frequency utility support for `list-disc`, numeric/arbitrary `line-clamp-*`, numeric/theme/arbitrary `columns-*`, and `aspect-*` with default `--aspect-video`. +4 tests. |
| Phase 24: Utility follow-up — grid placement/tracks | ✅ **Done** | Added grid utility support for column/row placement and spans, start/end lines, auto columns/rows, grid auto-flow, and grid template columns/rows. +6 tests. |
...
- **Phase 14 (Theme parity)**: rewrote `theme.rs` to mirror upstream `Theme` class. Theme entries store a value + bit-flag options (`NONE`/`INLINE`/`REFERENCE`/`DEFAULT`/`STATIC`/`USED`). `add()` honours namespace wildcards (`--font-*: initial`, `--*: initial`) and the `DEFAULT` precedence rule (existing user value wins). `resolve()` returns either `var(--…)` (with `REFERENCE` fallback) or the inline value (when entry/options carry `INLINE`); candidate-value fallback retries `1.5` → `1_5`. The upstream `ignoredThemeKeyMap` is ported (so e.g. `--font` does not match `--font-weight-*`/`--font-size-*` keys). Added `keys_in_namespaces`, `clear_namespace`, `namespace`, `mark_used_variable`, `entries_sorted`, prefix support (`prefix_key`/`unprefix_key`), and keyframe collection (`add_keyframes`/`get_keyframes`). Back-compat helpers `lookup_var` and `resolve_by_key_path` preserved so existing callers (`functions.rs`, tests) keep working.
- **Phase 15 (Variants parity)**: rewrote `variants.rs` with full upstream parity. The variant pipeline now supports at-rule wrapping (responsive breakpoints sm/md/lg/xl/2xl and their `max-*` inverses, `dark`/`print`/`motion-*`/`portrait`/`landscape`/`contrast-*`, arbitrary `min-[…]`/`max-[…]`, container queries `@container`/`@sm`–`@7xl`/`@min-[…]`/`@max-[…]`/`@[…]`, `supports-[…]`), functional variants (`data-*`, `aria-*`, `has-*`, `not-*`), `group-*`/`peer-*` (named + arbitrary with `&`), and arbitrary selector variants (`[&_p]`).
- **Phase 16 (Utilities — first wave)**: added a `FunctionalHandler` infrastructure to `UtilityRegistry` plus `Theme::with_defaults()` that seeds the v4 spacing base, default color palette, radius/text/font-weight scales. Implemented the spacing-driven utilities (margin/padding/gap/inset/edges in all axes), sizing utilities, the colour family with `/<opacity>` opacity modifiers (`color-mix(in oklab, ‹colour› ‹α›, transparent)`), border radius (`rounded` + per-corner variants), border widths, opacity, z-index, order, and flex grow/shrink/basis. The functional registry uses longest-prefix lookup so multi-segment keys like `rounded-tl-lg` resolve correctly to the `rounded-tl` handler with leftover `lg`.
