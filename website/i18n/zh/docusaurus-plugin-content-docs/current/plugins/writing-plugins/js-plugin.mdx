...
以下是 Farm v2 `JsPlugin` 接口中可用的所有 hooks。详细信息请参阅 [Js Plugin API](/zh/docs/api/js-plugin-api)。

**配置和服务器 Hooks：**
- `config` — 在编译前修改已解析的配置
- `configResolved` — 在配置完全解析后调用
- `configureServer` — 配置开发服务器（添加中间件等）
- `configureCompiler` — 访问和配置编译器实例
...
- `load` — 自定义模块加载
- `transform` — 转换模块内容
- `processModule` — 在转换后处理模块
- `freezeModule` — 模块冻结（构建期间最终化）后调用
- `buildEnd` — 构建结束时调用

**生成 Hooks：**
- `renderStart` — 资源生成开始时调用
- `processRenderedResourcePot` — 渲染后处理 resource pot
- `augmentResourcePotHash` — 增强 resource pot 的哈希值
- `finalizeResources` — 在输出前最终化所有资源
- `transformHtml` — 转换生成的 HTML
- `writeResources` — 资源写入磁盘时调用

**缓存 Hooks：**
- `pluginCacheLoaded` — 从磁盘加载插件缓存时调用
- `writePluginCache` — 将插件缓存写入磁盘时调用

**生命周期 Hooks：**
- `finish` — 编译完成时调用
- `updateModules` — HMR 期间确定需要更新哪些模块时调用
...
`Module_type `由 `load` 钩子返回，并且可以由 `transform` 钩子转换。Farm 原生支持 `js/ts/jsx/tsx`、`css`、`html`、`json`、`static assets(png、svg等)`。对于这些模块类型，你可以直接在 `load` 或 `transform` hook 中返回。但是如果你想处理自定义模块类型，你需要实现其他钩子例如 `parse`， `render_resource_pot`， `generate_resources` 等来控制如何对自定义模块进行类型解析，渲染和生成资源。
