...
* **`@farmfe/js-plugin-vue`**：支持编译 Vue SFC 文件。
* **`@farmfe/js-plugin-solid`**：支持编译 Solid JSX 文件。
* **`@farmfe/js-plugin-tailwindcss`**：支持 TailwindCSS 集成。
* **`@farmfe/js-plugin-visualizer`**：可视化分析产物大小与组成。
* **`@farmfe/js-plugin-electron`**：支持构建 Electron 应用。
