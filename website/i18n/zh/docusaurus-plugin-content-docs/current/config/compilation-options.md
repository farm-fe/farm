...
在输出入口文件中保留顶层 `await`。在构建需要保留异步初始化的库时非常有用。

#### `script.importNotUsedAsValues`

- **默认值**: `"remove"`

控制仅类型导入的处理方式。对应 TypeScript 的 [`importsNotUsedAsValues`](https://www.typescriptlang.org/tsconfig/#importsNotUsedAsValues)。可选值：

- `"remove"`：完全移除仅类型导入（默认）。
- `"preserve"`：保留所有导入。
- `{ preserve: string[] }`：一个包含 `preserve` 正则字符串数组的对象。匹配的模块按 `"preserve"` 处理，其余按 `"remove"` 处理。
...
配置 CSS Modules 导出到 JS 时本地变量名的转换方式。可选值：

- `"asIs"`：不转换，保持原始类名。
- `"lowerCamel"`：转换为小驼峰，例如 `foo-bar` → `fooBar`。
- `"upperCamel"`：转换为大驼峰，例如 `foo-bar` → `FooBar`。
- `"snake"`：转换为蛇形命名，例如 `foo-bar` → `foo_bar`。
...
是否将 CSS 模块转换为 JS 脚本。设为 `true` 时，每个 CSS 模块会被转换为通过 `document.createElement('style')` 注入到 document head 的脚本。这使开发模式下的逐模块 HMR 成为可能。生产构建中，CSS 通常保留为独立的 CSS 文件。
