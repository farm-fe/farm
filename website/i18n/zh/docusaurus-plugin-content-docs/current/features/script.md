...
## 原生顶层 Await
Farm 通过 [`compilation.script.nativeTopLevelAwait`](/zh/docs/config/compilation-options#scriptnativetoplevelawait) 支持原生顶层 `await`。启用后，Farm 会保留模块顶层的 `await` 而不是将其转换。
...
:::note
目标环境必须支持顶层 await（如现代浏览器、Node.js 14.8+）。
:::

## 仅类型导入行为
使用 [`compilation.script.importNotUsedAsValues`](/zh/docs/config/compilation-options#scriptimportnotusedasvalues) 控制仅类型导入的行为，对应 TypeScript 的 `--importsNotUsedAsValues` 标志。选项：

- `remove` — 删除仅导入类型的 import 语句（默认）
- `preserve` — 保留所有 import 语句
