...
`Farm` 通过dotenv-expand扩展环境变量。Farm 按以下顺序加载环境变量文件（后加载的文件会覆盖先加载的）：

1. `.env` — 所有情况下都会加载
2. `.env.local` — 所有情况下都会加载，被 git 忽略
3. `.env.{mode}` — 仅在指定模式下加载（如 `.env.development`、`.env.production`）
4. `.env.{mode}.local` — 仅在指定模式下加载，被 git 忽略

对于仅用于开发的环境变量，使用 `.env.development` 文件，对于仅用于生产的环境变量，使用 `.env.production` 文件，对于通过 `--mode <stage>` 传递的自定义模式，从 `.env.<stage>` 文件加载。
