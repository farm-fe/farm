...
### 自定义环境变量 (`envs`)
使用 `persistentCache.envs` 指定影响缓存键的额外环境变量键值对。当这些值中的任何一个发生变化时，缓存将完全失效：
...
### 全局内置缓存键策略 (`globalBuiltinCacheKeyStrategy`)
通过 `persistentCache.globalBuiltinCacheKeyStrategy` 控制哪些内置键影响缓存失效。默认情况下，`define`、`buildDependencies`、`lockfile`、`packageJson` 和 `env` 模式的变化都会使整个缓存失效。你可以按需禁用各个键：
  ...
  compilation: {
    persistentCache: {
      globalBuiltinCacheKeyStrategy: {
        define: true,            // 当 define 配置变化时使缓存失效
        buildDependencies: true, // 当构建依赖变化时使缓存失效
        lockfile: true,          // 当 lockfile 变化时使缓存失效
        packageJson: true,       // 当 package.json 变化时使缓存失效
        env: true,               // 当环境模式变化时使缓存失效
      },
    },
  },
});
