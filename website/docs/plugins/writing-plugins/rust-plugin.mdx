...
The following hooks are available in Farm v2's `Plugin` trait. See [Rust Plugin API](/docs/api/rust-plugin-api) for full details on each hook.

**Config Hooks:**
- `config` — Modify the resolved config before compilation
- `plugin_cache_loaded` — Called when plugin cache is loaded from disk

**Build Hooks:**
- `build_start` — Called when the build starts
- `resolve` — Custom module resolution
- `load` — Custom module loading
- `transform` — Transform module content
- `parse` — Parse a module into AST
- `process_module` — Process a module after parsing
- `analyze_deps` — Analyze the dependencies of a module
- `finalize_module` — Finalize a module before freezing
- `freeze_module` — Called when a module is frozen (v2)
- `module_graph_build_end` — Called when the module graph build is complete (v2)
- `build_end` — Called when the build is complete

**Generate Hooks:**
- `generate_start` — Called when resource generation starts
- `optimize_module_graph` — Optimize the module graph before bundling
- `freeze_module_graph_meta` — Freeze module graph metadata (v2)
- `analyze_module_graph` — Analyze the module graph for bundling
- `partial_bundling` — Custom partial bundling logic
- `process_resource_pots` — Process resource pots after bundling (v2)
- `render_start` — Called when rendering starts
- `render_resource_pot` — Render a single resource pot
- `process_rendered_resource_pot` — Process a resource pot after rendering (v2)
- `augment_resource_pot_hash` — Augment the hash of a resource pot (v2)
- `optimize_resource_pot` — Optimize a rendered resource pot
- `generate_resources` — Generate final resources from resource pots
- `process_generated_resources` — Process resources after generation (v2)
- `handle_entry_resource` — Handle entry resource processing (v2)
- `finalize_resources` — Finalize all resources before output
- `generate_end` — Called when generation is complete

**Lifecycle Hooks:**
- `finish` — Called when the compilation is finished
- `write_plugin_cache` — Called to write plugin cache to disk

**HMR Hooks:**
- `update_modules` — Called during HMR to determine which modules to update
- `module_graph_updated` — Called after the module graph is updated during HMR (v2)
- `update_finished` — Called when HMR update is finished (v2)
- `handle_persistent_cached_module` — Handle loading a module from persistent cache (v2)
...
`module_type` returned by `load` hook, and can be transformed by `transform` hook. Farm supports `js/ts/jsx/tsx`, `css`, `html`, `json`, `static assets(png, svg, etc)` natively. For these module types, you can return them directly in `load` or `transform` hook directly. But if you want to handle custom module types, you may need to implement other hooks like `parse`, `render_resource_pot`, `generate_resources`, etc to control how to parse, render and generate_resources for the custom module types.
...
<Tabs>
      <TabItem value="pnpm" label="pnpm">
          ...
          <TabItem value="npm" label="npm">
              <CodeBlock>npm create farm-plugin@latest my-farm-plugin -- --type rust</CodeBlock>
              </TabItem>

  <TabItem value="yarn" label="yarn">
          <CodeBlock>yarn create farm-plugin my-farm-plugin --type rust</CodeBlock>
              </TabItem>
          </Tabs>
...
```json
{
  ...
  "scripts": {
    ...
    "build": "farm-plugin-tools build --platform -p my_farm_plugin --release",
...
For non-native supported module types, you should use `transform` hook to transform the module type to a native supported module type, otherwise you need to implement `parse`, `render_resource_pot` hook to handle your custom module type:
...
impl Plugin for FarmPluginExample {
  ...
  fn transform(
    ...
    if matches!(&param.module_type, farmfe_core::module::ModuleType::Custom(name) if name == "sass") {
      ...
      Ok(Some(farmfe_core::plugin::PluginTransformHookResult {
        ...
        content: compile_sass(&param.content),
        ..Default::default()
      }))
    } else {
      Ok(None)
...
:::note
Module type guard like `matches!(&param.module_type, farmfe_core::module::ModuleType::Custom(name) if name == "sass")` is required in the `transform` hook, because the `transform` hook will be called for all module types, and you should only handle your custom module type in the `transform` hook. Do the same in `parse` and other hooks.
:::


or implement `parse`, `render_resource_pot` hook to handle your custom module type, see how native farm css plugin handle `css` module type in [farm-plugin-css](https://github.com/farm-fe/farm/blob/main/crates/plugin_html/src/lib.rs#L159).
