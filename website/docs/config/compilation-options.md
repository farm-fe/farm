...
interface OutputOptions {
  ...
  // output ascii only
  asciiOnly?: boolean;
  // external globals name mapping, only works when targetEnv is 'browser' or 'library' with format 'iife' or 'umd'
  externalGlobals?: Record<string, string>;
  // global name for umd/iife format
  name?: string;
}
```
...
Configure the output module format. Supported values: `"esm"`, `"cjs"`, `"umd"`, `"iife"`, `"system"`, `"amd"`.

When `output.targetEnv` is `"library"`, you can pass an array of formats to generate multiple outputs in a single build:
...
Whether to print file size information for the final output files after build.
...
Map external module names to global variable names. For example, `{ "react": "React" }` means `import * as React from 'react'` will access `React` from `window.React` at runtime.

:::note
Only works when `targetEnv` is `browser`, or `library` with format `iife` or `umd`.
:::

#### `output.name`

- **default**: `"__farm_global__"`

The global variable name used for `umd`/`iife` format output. If not set, the default `__farm_global__` will be used.
...
- **default**: `["tsx", "mts", "cts", "ts", "jsx", "mjs", "js", "cjs", "json", "html", "css"]`
...
You can also use the array form for more control:
  ...
  compilation: {
    resolve: {
      alias: [
        ...
        { find: /^\/(utils)$/, replacement: path.join(process.cwd(), 'src/$1') },
      ],
    },
  },
});
...
- **default**: `["development", "production", "module"]`

The [conditions](https://nodejs.org/api/packages.html#conditional-exports) field used when resolving package `exports`/`imports` in `package.json`.
...
Whether to strictly follow the exports defined in `exports` in `package.json`. If set to true, when `exports` is defined in `package.json`, but `exports` does not define the corresponding export, an error will be reported directly. If set to false, it will continue to try other entries according to mainFields.

#### `resolve.autoExternalFailedResolve`

- **default**: `false`

If a module cannot be resolved, automatically mark it as external instead of raising an error.

#### `resolve.dedupe`

- **default**: `[]`

Configure a list of module names that should always be resolved to the same copy from the project root, even if multiple versions exist in the dependency tree. Useful for avoiding duplicated packages like `react`.
...
:::warning
It is not recommended to configure this option under normal circumstances, because once this option is configured, the pointed runtime needs to be compatible with Farm's internal runtime.
:::
...
Configure the Runtime plug-in, through the Runtime plug-in, you can intervene in Runtime behavior, such as module loading, resource loading, etc. For details, refer to [Runtime Plugin API](/docs/api/runtime-plugin-api).

#### `runtime.swcHelpersPath`

- **default**: `undefined`

Customize the path of `@swc/helpers`. It is not recommended to set this option unless you have a specific need to override the default helpers resolution.
...
#### `script.nativeTopLevelAwait`

- **default**: `false`

Keep top-level `await` in the output entry file. Useful when building a library that needs to preserve async initialization.

#### `script.importNotUsedAsValues`

- **default**: `"remove"`

Controls how type-only imports are handled. Mirrors TypeScript's [`importsNotUsedAsValues`](https://www.typescriptlang.org/tsconfig/#importsNotUsedAsValues). Possible values:

- `"remove"`: remove type-only imports entirely (default).
- `"preserve"`: keep all imports.
- `{ preserve: string[] }`: an object with a `preserve` array of regex strings. Modules matching any regex are treated as `"preserve"`; all others are `"remove"`.
...
interface FarmCssModulesConfig {
  ...
  indentName?: string;
  // configure the local variable name conversion for css modules
...
##### `css.modules.localsConversion`

- **default**: `"asIs"`

Configure how CSS Modules local variable names are converted when exported to JS. Possible values:

- `"asIs"`: no conversion, keep the original class name.
- `"lowerCamel"`: convert to lower camelCase, e.g. `foo-bar` → `fooBar`.
- `"upperCamel"`: convert to upper CamelCase, e.g. `foo-bar` → `FooBar`.
- `"snake"`: convert to snake_case, e.g. `foo-bar` → `foo_bar`.
...
Whether to transform CSS modules to JS scripts. When `true`, each CSS module will be converted to a script that injects the CSS into the document head via `document.createElement('style')`. This enables per-module HMR in development. In production builds, CSS is typically kept as standalone CSS files.
...
- **default**: `{ define: true, buildDependencies: true, lockfile: true, packageJson: true, env: true }`

Controls whether Farm's built-in cache keys are included when computing the global cache key. If any of these built-in keys change, the entire cache is fully invalidated. You can selectively disable keys that you know won't affect your build:

- **`define`**: Whether changes to `compilation.define` invalidate cache.
- **`buildDependencies`**: Whether changes to `buildDependencies` invalidate cache.
- **`lockfile`**: Whether changes to the lockfile (e.g. `pnpm-lock.yaml`) invalidate cache.
- **`packageJson`**: Whether changes to `package.json` invalidate cache.
- **`env`**: Whether changes to environment variables invalidate cache.
...
Whether to enable module concatenation (scope hoisting). When enabled, Farm will merge modules into a single scope where possible, similar to webpack's `ModuleConcatenationPlugin`. This can reduce bundle size and improve runtime performance by eliminating module wrapper overhead.

:::warning
When `concatenateModules` is `true`, all `runtime` options (such as `runtime.plugins`, `runtime.path`, etc.) are ignored because concatenated modules are not wrapped by the Farm runtime.
:::
...
Whether to record compilation flow statistics. When enabled, Farm collects internal data about the compilation process, useful for debugging and performance analysis.
...
Reserved configuration field for passing custom key-value pairs. Can be used to pass arbitrary data to plugins or for future compatibility.
