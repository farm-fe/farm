...
### Custom Env Variables for Cache (`envs`)
Use `persistentCache.envs` to specify additional environment variable key-value pairs that affect the cache key. When any of these values change, the cache is fully invalidated:
...
### Global Builtin Cache Key Strategy (`globalBuiltinCacheKeyStrategy`)
Control which built-in keys affect cache invalidation via `persistentCache.globalBuiltinCacheKeyStrategy`. By default, changes to `define`, `buildDependencies`, `lockfile`, `packageJson`, and `env` mode all invalidate the entire cache. You can disable individual keys if needed:
  ...
  compilation: {
    persistentCache: {
      globalBuiltinCacheKeyStrategy: {
        define: true,            // invalidate cache when `define` config changes
        buildDependencies: true, // invalidate cache when build dependencies change
        lockfile: true,          // invalidate cache when lockfile changes
        packageJson: true,       // invalidate cache when package.json changes
        env: true,               // invalidate cache when env mode changes
      },
    },
  },
});
