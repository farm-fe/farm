...
Farm supports native top-level `await` via [`compilation.script.nativeTopLevelAwait`](/docs/config/compilation-options#scriptnativetoplevelawait). When enabled, Farm will preserve `await` at the top level of modules instead of transforming it away.
...
:::note
Your target environment must support top-level await (e.g. modern browsers, Node.js 14.8+).
:::

## Import Not Used As Values
Use [`compilation.script.importNotUsedAsValues`](/docs/config/compilation-options#scriptimportnotusedasvalues) to control the behavior of type-only imports, mirroring TypeScript's `--importsNotUsedAsValues` flag. Options:

- `remove` — Drop import statements that only import types (default)
- `preserve` — Keep all import statements
