...
<img src="https://github.com/user-attachments/assets/f7ce7d67-542b-41c8-a432-205c9a01c33b" />
...
- `biome` farm use biome as lint and format the code
...
- Work with examples (open a new terminal): `cd examples/refactor-react && pnpm start`, report an issue if the example does not start normally.

- If `examples/refactor-react` project runs successfully, the development environment has been configured successfully
...
If you want to develop a plugin for Farm, use the standalone scaffold to quickly create a JS or Rust plugin:

```bash
$ pnpm create farm-plugin <plugin-name> # create a plugin supporting JS or Rust
```
