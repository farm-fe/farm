...
export default defineConfig({
  // ...
  plugins: [
    // Rust plugin, import its default export and call it
    sass(),
...
### Using Rust Plugins
Rust plugins are imported and called the same way as JS plugins. Each rust plugin package exports a default factory function:
```ts title="farm.config.ts"
import sass from "@farmfe/plugin-sass";
...
```
Pass options as an argument of the factory:
```ts title="farm.config.ts"
import sass from "@farmfe/plugin-sass";
...
* **`@farmfe/plugin-sass`**: Farm rust plugin for scss files compilation, powered by `grass`.
...
## Using SWC Plugins
Swc Plugin can also be used directly in Farm, Configuring `compilation.script.plugins` to add SWC plugins, for example. For Vue SFC support, register the current Rust plugin `@farmfe/plugin-vue` in the top-level `plugins` array:
```ts
...
:::note
`SWC plugin` may not be compatible with the `SWC version`(rust crate `swc_core v59.0.1`) that Farm uses. If a error occurred, try upgrade the plugin version. 
:::
