...
mod tests {
    ...
    #[test]
    fn string_true_returns_true() {
        assert!(resolve_debug(Some("true")));
    }

    #[test]
    fn string_1_returns_true() {
        assert!(resolve_debug(Some("1")));
    }

    #[test]
    fn string_false_returns_false() {
        assert!(!resolve_debug(Some("false")));
    }

    #[test]
    fn string_0_returns_false() {
        assert!(!resolve_debug(Some("0")));
    }

    // ── Wildcard ─────────────────────────────────────────────────────────────

    #[test]
    fn wildcard_returns_true() {
        assert!(resolve_debug(Some("*")));
    }

    // ── Comma-separated list with tailwindcss ────────────────────────────────

    #[test]
    fn list_includes_tailwindcss_returns_true() {
        assert!(resolve_debug(Some("tailwindcss")));
        assert!(resolve_debug(Some("projectA,tailwindcss,projectB")));
    }

    #[test]
    fn list_with_namespace_colon_star_returns_true() {
        // "tailwindcss:*" → first segment before `:` is "tailwindcss"
        assert!(resolve_debug(Some("tailwindcss:*")));
        assert!(resolve_debug(Some("other,tailwindcss:submodule")));
    }

    #[test]
    fn list_excludes_tailwindcss_returns_false() {
        assert!(!resolve_debug(Some("-tailwindcss")));
        assert!(!resolve_debug(Some("projectA,-tailwindcss")));
    }

    #[test]
    fn exclusion_takes_precedence_over_inclusion() {
        // If -tailwindcss appears anywhere, return false
        assert!(!resolve_debug(Some("tailwindcss,-tailwindcss")));
    }

    #[test]
    fn unrelated_list_returns_false() {
        assert!(!resolve_debug(Some("projectA,projectB")));
    }
