...
static CSS_URL_RE: LazyLock<Regex> = LazyLock::new(|| {
  ...
  Regex::new(r#"(?:^|[^\w\-\x{0080}-\x{ffff}])url\((\s*('[^']+'|"[^"]+")\s*|[^'")\s]+)\)"#).unwrap()
});

static CSS_IMAGE_SET_RE: LazyLock<Regex> =
  LazyLock::new(|| Regex::new(r#"image-set\(((?:[^)]*(?:\([^)]*\))?)*)\)"#).unwrap());

static CSS_URL_INNER_RE: LazyLock<Regex> =
  LazyLock::new(|| Regex::new(r#"url\((\s*('[^']+'|"[^"]+")\s*|[^'")\s]+)\)"#).unwrap());

static DATA_URL_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r#"(?i)^\s*data:"#).unwrap());
