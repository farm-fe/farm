...
Regenerated after merging `origin/main`: the `Cargo.lock` conflict was resolved from main and then regenerated by targeted Cargo validation so the current `farmfe_plugin_vue` package and git-pinned fervid dependency are represented. The plugin implementation, docs, changeset, OpenSpec artifacts, and plugin-vue workflow artifact wiring are kept on this branch.
...
- [x] 6.1. `.github/workflows/rust-build.yaml` — add plugin-vue to the `npm run build` chain in the multi-target build step; add an `actions/upload-artifact` for `rust-plugins/vue/npm/${{ matrix.settings.abi }}/index.farm` under the name `${{ github.sha }}-${{ matrix.settings.abi }}-plugin-vue`.
- [x] 6.2. `.github/workflows/ci.yaml` — add a `download-plugin-vue` artifact step in the per-abi examples job, downloading into `./rust-plugins/vue/npm/${{ matrix.settings.abi }}`. Add `plugin-vue` to the rust-plugin artifact check matrix list.
- [x] 6.3. `.github/workflows/release.yaml` — extend the artifact-move and `test -f` existence checks to cover `./rust-plugins/vue/npm/${abi}/index.farm`.
