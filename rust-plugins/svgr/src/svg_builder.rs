...
impl SvgBuilder {
  ...
  pub fn fill(mut self, fill: Option<String>) -> Self {
    if let Some(fill_value) = fill {
      fn update_fill_recursive(element: &mut Element, fill: &str) {
        element
          .attributes
      ...
      update_fill_recursive(&mut self.element, &fill_value);
    }
    self
  }
  ...
  pub fn remove_fill_attributes(mut self) -> Self {
    fn remove_fill_recursive(element: &mut Element) {
      element.attributes.remove("fill");

      for child in element.children.iter_mut() {
        if let Some(child_element) = child.as_mut_element() {
          remove_fill_recursive(child_element);
        }
      }
    }

    remove_fill_recursive(&mut self.element);
    self
  }
...
mod tests {
  ...
  fn test_remove_fill_attributes() {
    ...
    // let builder = SvgBuilder::new(svg_content);
    // let result = builder
    //     .remove_fill_attributes()
    //     .build();

    // assert!(!result.contains(r#"fill="#00D8FF""#));
