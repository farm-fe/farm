...
pub fn generate_dts(option: GenerateDtsOption) {
  ...
  let mut entries = Vec::with_capacity(option.imports.len() + option.presets_imports.len());

  entries.extend(option.imports.iter().map(|&item| DtsEntry {
    declaration_name: item.as_name.clone().unwrap_or(item.name.clone()),
    source: item.from.clone(),
    code: stringify_imports_dts(dts_parent, item),
  }));
  entries.extend(option.presets_imports.iter().map(|&item| DtsEntry {
    declaration_name: item.as_name.clone().unwrap_or(item.name.clone()),
    source: item.from.clone(),
    code: stringify_presets(item),
  }));
