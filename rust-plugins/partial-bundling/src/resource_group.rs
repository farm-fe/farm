...
impl ResourceGroup {
  ...
  pub fn remove_resource_group(&mut self, resource_unit_id: &ResourceUnitId) {
      ...
      |resource_unit_group_id: ResourceUnitId| {
        ...
        if group_modules.is_empty() {
          ...
          unit_group
            ...
            .iter()
            .for_each(|module_id| {
              group_modules.extend(
                module_group_map[module_id]
                  .iter()
                  .cloned()
                  .collect::<HashSet<_>>(),
              );
  ...
  pub fn clean_empty_resources(&mut self) {
    ...
    empty_resources.iter().for_each(|resource| {
      self.remove_resource_group(resource);
    });
  }
}
