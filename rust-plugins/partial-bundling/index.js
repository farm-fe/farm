      ...
      case 'x64':
        if (existsSync(join(currentDir, './npm/win32-x64-msvc/index.farm'))) {
          ...
          binPath = require.resolve(
            '@farmfe/plugin-webpack-partial-bundling-win32-x64-msvc'
          );
        }
    ...
    switch (arch) {
      case 'x64':
        if (existsSync(join(currentDir, './npm/darwin-x64/index.farm'))) {
          ...
          binPath = require.resolve('@farmfe/plugin-webpack-partial-bundling-darwin-x64');
        }
        break;
      case 'arm64':
        if (existsSync(join(currentDir, './npm/darwin-arm64/index.farm'))) {
          ...
          binPath = require.resolve('@farmfe/plugin-webpack-partial-bundling-darwin-arm64');
        }
        break;
      default:
    ...
    switch (arch) {
      case 'x64':
        if (existsSync(join(currentDir, './npm/linux-x64-gnu/index.farm'))) {
          ...
          binPath = require.resolve(
            '@farmfe/plugin-webpack-partial-bundling-linux-x64-gnu'
          );
        }
        break;
      default:
