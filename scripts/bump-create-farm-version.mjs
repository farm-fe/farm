...
const packageReactPluginJson = JSON.parse(
  fs.readFileSync(packageReactPluginPath, "utf8"),
);
      ...
      fs.stat(fullPath, (err, stats) => {
        if (err) {
          return console.error(
            `cannot get stats of file ${fullPath}: ${err.message}`,
          );
        }
        ...
        if (stats.isDirectory()) {
          ...
          fs.readFile(fullPath, "utf8", (err, data) => {
            if (err) {
              return console.error(
                `cannot read file ${fullPath}: ${err.message}`,
              );
            }
            ...
            try {
              ...
              return console.error(
                `resolve JSON file ${fullPath} error: ${err.message}`,
              );
            }
            ...
            fs.writeFile(
              ...
              (err) => {
                if (err) {
                  return console.error(
                    `cannot write ${fullPath}: ${err.message}`,
                  );
                }
