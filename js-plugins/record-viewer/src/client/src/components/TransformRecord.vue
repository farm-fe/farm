<template>
  <div class="w-full h-full flex p-2 gap-3 box-border overflow-hidden">
    <div class="flex flex-col border rounded-lg box-border">
      <div v-for="(item, index) in records" class="flex items-center cursor-pointer text-base font-semibold text-gray-900 px-3 py-4 border-b" :class="{'text-purple-500': current === index}" @click="current = index">
        <div>{{ item.plugin }}</div>
        <div v-if="item.hook === 'load'" class="ml-2 border text-purple-500 border-purple-500 px-2 rounded-md">load</div>
        <!-- <div class="record-details">{{ item.plugin }} | {{item.kind}}</div> -->
      </div>
    </div>
