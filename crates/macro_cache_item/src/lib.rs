...
use crate::common::CacheItemArgs;
...
#[proc_macro_attribute]
pub fn cache_item_options(attr: TokenStream, input: TokenStream) -> TokenStream {
  let item: Item = parse_macro_input!(input);
  let CacheItemArgs { crate_name, derive } = parse_macro_input!(attr as CacheItemArgs);
  let derive = derive.unwrap_or(parse_quote! { derive() });
