...
pub fn create_basic_graph() -> ModuleGraph {
  ...
  module_graph
    ...
    .unwrap();
  module_graph.entries = FxHashMap::from_iter([("a".into(), "a".to_string())]);
...
fn test_diff_module_deps_2() {
  ...
  let (diff_result, added_modules, removed_modules) =
    super::diff_module_deps(&vec!["a".into()], &module_graph, &update_module_graph);
  assert_eq!(added_modules, FxHashSet::from_iter(["f".into()]));
  assert_eq!(
    removed_modules,
    FxHashSet::from_iter(["b".into(), "c".into()])
  );
...
fn test_diff_module_deps_3() {
  ...
  let (diff_result, added_modules, removed_modules) =
    super::diff_module_deps(&vec!["a".into()], &module_graph, &update_module_graph);
  assert_eq!(added_modules, FxHashSet::from_iter(["d".into()]));
  assert!(removed_modules.is_empty());
...
fn test_diff_module_deps_4() {
  ...
  let (diff_result, added_modules, removed_modules) =
    super::diff_module_deps(&vec!["a".into()], &module_graph, &update_module_graph);
  assert!(added_modules.is_empty());
  assert_eq!(removed_modules, FxHashSet::from_iter(["d".into()]));
...
fn test_diff_module_deps_complex_2() {
  ...
  assert_eq!(
    removed_modules,
    FxHashSet::from_iter(["E".into(), "G".into()])
  );
...
fn test_diff_module_deps_complex_4() {
  ...
  assert!(added_modules.is_empty());
  assert_eq!(removed_modules, FxHashSet::from_iter(["G".into()]));
  assert_eq!(
    diff_result,
    Vec::from([(
      "E".into(),
...
fn test_diff_module_deps_add_and_remove() {
  ...
  let (diff_result, added_modules, removed_modules) =
    super::diff_module_deps(&vec!["a".into()], &module_graph, &update_module_graph);
  assert_eq!(added_modules, FxHashSet::from_iter(["e".into()]));
  assert_eq!(removed_modules, FxHashSet::from_iter([]));
  assert_eq!(
    diff_result,
    Vec::from([
      (
...
fn test_diff_module_deps_remove_and_add() {
  ...
  let (diff_result, added_modules, removed_modules) =
    super::diff_module_deps(&vec!["a".into()], &module_graph, &update_module_graph);
  assert_eq!(removed_modules, FxHashSet::from_iter(["e".into()]));
  assert_eq!(added_modules, FxHashSet::from_iter([]));
  assert_eq!(
    diff_result,
    Vec::from([
      (
...
fn test_diff_module_deps_remove_and_add_complex() {
  ...
  assert_eq!(
    added_modules,
    FxHashSet::from_iter(["e".into(), "c".into()])
  );
  assert_eq!(removed_modules, FxHashSet::from_iter([]));
  assert_eq!(
    diff_result,
