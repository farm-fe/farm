...
#[test]
fn test_patch_module_group_graph_dynamic_entry_remove_dynamic_entry() {
  let mut module_graph = construct_test_module_graph_complex();
  let mut update_module_graph = construct_test_module_graph_complex();
  module_graph
    .update_edge(
  ...
  assert_eq!(module_graph.dynamic_entries, HashMap::default());
  ...
  assert_eq!(
    module_h.module_groups,
    HashSet::from_iter([group_id_f.clone(), group_id_g.clone(), group_id_b.clone()])
  );
}
