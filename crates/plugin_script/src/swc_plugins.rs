        ...
        if let Some(plugins) = &mut self.plugins {
          for p in plugins.drain(..) {
            ...
            serialized = plugin_transform_executor
              ...
              .with_context(|| {
                format!(
                  "failed to invoke `{}` as js transform plugin at {plugin_name}. {plugin_name} may not be compatible with current version of rust crate swc_core(v0.95.6) that Farm uses, please upgrade or downgrade version of swc plugin {plugin_name} to match the swc_core version.",
                  &p.name
                )
              })?;
          }
