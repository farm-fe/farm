...
#[test]
fn hmr_accepted_deps() {
  fixture!("tests/fixtures/hmr_accepted_deps/*.ts", |file, _| {
    let config = Config::default();
    let plugin_script = farmfe_plugin_script::FarmPluginScript::new(&config);
    let context = Arc::new(CompilationContext::new(config, vec![]).unwrap());
    let id = file.to_string_lossy().to_string();
    ...
    assert!(!module.meta.as_script().hmr_self_accepted);
    assert!(module.meta.as_script().hmr_accepted_deps.is_empty());
    plugin_script
      .finalize_module(
        &mut PluginFinalizeModuleHookParam {
          module: &mut module,
    ...
    assert!(!module.meta.as_script().hmr_self_accepted);
    assert_eq!(
      module.meta.as_script().hmr_accepted_deps,
      HashSet::from(["./foo.js".into(), "./bar.js".into()])
    );
  });
}
