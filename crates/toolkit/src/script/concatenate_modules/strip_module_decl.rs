    ...
    if let Some(import_info) = &statement.import_info {
      ...
      for specifier in &import_info.specifiers {
        ...
        if rename_imported_ident(
            ...
            .get(export_str)
          {
            if ambiguous_export_idents.len() > 1 {
              println!(
                ...
                ambiguous_export_idents[0].ident.sym,
                ambiguous_export_idents[0].module_id.to_string(),
              );
            }
            ...
            if ambiguous_export_idents.len() > 0 {
              let module_export_ident = &ambiguous_export_idents[0];
          ...
          {
            export_namespace_ident.ident.clone()
          } else {
