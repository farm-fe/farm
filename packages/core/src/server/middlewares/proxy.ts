...
import { IBaseKoaProxiesOptions, IKoaProxiesOptions, default as koaProxy } from 'koa-proxies';
...
export function useProxy(options: UserConfig['server']['proxy'], app: Application, logger: Logger) {
  for (const path of Object.keys(options)) {
    ...
    const proxyMiddleware = koaProxy(path[0] === '^' ? new RegExp(path) : path, opts);
