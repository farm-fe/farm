  ...
  constructor(pluginName: string, config: any, server: any) {
    ...
    return new Proxy(this, {
      get: (_target, prop) => {
        switch (prop) {
          case 'moduleGraph':
            return createViteModuleGraphAdapter(pluginName);
          case 'watcher':
            return this._server.watcher.getInternalWatcher();
          case 'middlewares':
            return this._server.middlewares &&
              typeof this._server.middlewares.use === 'function'
              ? this._server.middlewares
              : { use: () => config.logger.warn('Middlewares not available') };
          default: {
